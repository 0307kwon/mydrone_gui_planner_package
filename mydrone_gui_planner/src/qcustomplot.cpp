...
#include <mydrone_gui_planner/qcustomplot.hpp>
...
/*! \fn void QCustomPlot::selectionChangedByUser()
  ...
  In addition to this signal, selectable objects also provide individual Q_SIGNALS, for example
  QCPAxis::selectionChanged or QCPAbstractPlottable::selectionChanged. Note that those Q_SIGNALS are
...
/*! \internal
 ...
 Event handler for when a double click occurs. Q_EMITs the \ref mouseDoubleClick signal, then Q_EMITs
 the specialized Q_SIGNALS when certain objecs are clicked (e.g. \ref plottableDoubleClick, \ref
...
/*! \internal
  ...
  Event handler for when a mouse button is pressed. Q_EMITs the mousePress signal. Then determines
...
void QCustomPlot::mouseReleaseEvent(QMouseEvent *event)
{
  ...
  if ((mMousePressPos-event->pos()).manhattanLength() < 5) // determine whether it was a click operation
  {
    ...
    // Q_EMIT specialized object click Q_SIGNALS:
    QVariant details;
    QCPLayerable *clickedLayerable = layerableAt(event->pos(), false, &details); // for these Q_SIGNALS, selectability is ignored, that's why we call this again with onlySelectable set to false
    if (QCPAbstractPlottable *ap = qobject_cast<QCPAbstractPlottable*>(clickedLayerable))
...
/*!
  ...
  and left axes are connected to the \ref QCPAxis::setRange Q_SLOTS of the top and right axes.
*/
...
/*! \class QCPPlotTitle
  ...
  Since a plot title is a common requirement, QCustomPlot offers specialized selection Q_SIGNALS for
...
void QCPColorScale::setType(QCPAxis::AxisType type)
{
  ...
  if (mType != type)
  {
    ...
    mColorAxis.data()->setScaleLogBase(logBaseTransfer); // scaleType is synchronized among axes in realtime via Q_SIGNALS (connected in QCPColorScale ctor), so we only need to take care of log base here
